...
exports.placeOrder = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            redirectUrl: `/user/success?customId=${newOrder.order_id}&orderId=${newOrder._id}`
        });
    } catch (error) {
...
exports.varifyPayment = async (req, res) => {
    try {
        ...
        if (generated_signature !== payment.razorpay_signature) {
            ...
            return res.status(400).json({ success: false, message: 'Payment verification failed.',redirectUrl: `/user/failure?orderId=${dbOrderId}&customId=${customId}` });
        }
        ...
        if (validProductsInOrder.length !== currentOrder.products.length) {
            await Order.updateOne({ _id: dbOrderId }, { $set: {payment_status: 'FAILED','products.$[].status': 'PENDING'}});
            return res.json({ success: false, message: `A product in your order is no longer available. Your payment will be refunded.`,redirectUrl: `/user/failure?orderId=${dbOrderId}&customId=${customId}` });
        }
        for (const item of validProductsInOrder) {
            ...
            if (!sizeVariant || sizeVariant.stock < item.quantity) {
                await Order.updateOne({ _id: dbOrderId }, { $set:{payment_status: 'FAILED','products.$[].status': 'PENDING'}});
                return res.json({ success: false, message: `Stock for ${product.title} is no longer available. Your payment will be refunded.`, redirectUrl: `/user/failure?orderId=${dbOrderId}&customId=${customId}` });
            }
        }
        ...
        await Cart.deleteOne({ userId: userId });
        delete req.session.coupon;
        res.json({ success: true, redirectUrl: `/user/success?customId=${customId}&orderId=${dbOrderId}` });
    } catch (error) {
