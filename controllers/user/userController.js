...
const formatProductForHomepage = (product) => {
    ...
    let displayImageUrl = '/uploads/products/placeholder.png'; // Assuming your upload path is /uploads/products/

    // Check if there are color variants
    if (product.colorVariants && product.colorVariants.length > 0) {
        let minPrice = Infinity;

        // Iterate through each color variant
        product.colorVariants.forEach(colorVariant => {
            ...
            // Iterate through size variants within this color variant to find the minimum price
        ...
        // Set displayPrice if a valid minimum price was found
...
exports.getHome = async (req, res) => {
    try {
        // Fetch products, then process them
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    console.log("Session User:", sessionUser); // 💥 Debug here
    ...
    console.log("Full User from DB:", user); // 💥 Debug here
